(()=>{
  ...
  function card(b){
    ...
    const client=b.perspective==='client'||b.clientPerspective===true;
    const canPay=client&&['pending','confirmed'].includes(b.status)&&!['paid','refunded'].includes(st);
    const canRefund=client&&st==='paid';
    return `<article class="zq-payment-card"><div><strong>${esc(b.serviceName||b.service_name||`Reserva #${id}`)}</strong><small>${money(total)} · ${esc(label(st))}</small></div><div class="zq-payment-actions">${canPay?`<button type="button" data-pay-booking="${esc(id)}">Pagar · prueba</button>`:''}<button type="button" class="secondary" data-payment-status="${esc(id)}">Estado</button>${['paid','refunded'].includes(st)?`<button type="button" class="secondary" data-payment-receipt="${esc(id)}">Comprobante</button>`:''}${canRefund?`<button type="button" class="secondary" data-payment-refund="${esc(id)}">Reembolso · prueba</button>`:''}</div></article>`;
  }
  ...
  async function status(id,btn){
    btn.disabled=true;
    try{const r=await fetch(`${API()}/api/payments/bookings/${encodeURIComponent(id)}`,{headers:{Authorization:`Bearer ${token()}`}});const d=await r.json();if(!r.ok)throw new Error(d.error||'No se pudo consultar');const p=d.payment;notice(`Estado: ${label(p?.status||d.paymentStatus||'not_started')}${p?` · Total ${money(p.grossAmount)} · Comisión ${money(p.platformFee)}`:''}`);}catch(e){notice(e.message,'error');}finally{btn.disabled=false;}
  }

  async function receipt(id,btn){
    btn.disabled=true;
    try{const r=await fetch(`${API()}/api/payments/bookings/${encodeURIComponent(id)}/receipt`,{headers:{Authorization:`Bearer ${token()}`}});const d=await r.json();if(!r.ok)throw new Error(d.error||'No se pudo cargar el comprobante');alert(`Comprobante de prueba\n${d.serviceName||`Reserva #${id}`}\nEstado: ${label(d.status)}\nTotal: ${money(d.total)}\nComisión Zeqviro: ${money(d.platformFee)}\nProveedor: ${money(d.providerAmount)}`);}catch(e){notice(e.message,'error');}finally{btn.disabled=false;}
  }

  async function refundTest(id,btn){
    if(!confirm('¿Confirmas el reembolso de PRUEBA? No afecta dinero real.')) return;
    btn.disabled=true; const old=btn.textContent; btn.textContent='Reembolsando…';
    try{const r=await fetch(`${API()}/api/payments/bookings/${encodeURIComponent(id)}/refund-test`,{method:'POST',headers:authHeaders(),body:JSON.stringify({confirm:true})});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'No se pudo reembolsar');notice('Reembolso de prueba procesado.');await load();}catch(e){notice(e.message,'error');btn.disabled=false;btn.textContent=old;}
  }

  async function handleReturn(attempt=0){
    const q=new URLSearchParams(location.search); const state=q.get('payment'); const sessionId=q.get('session_id');
    if(!state) return;
    if(state==='cancelled'){notice('Pago de prueba cancelado.');cleanPaymentQuery();return;}
    if(state!=='success'||!sessionId) return;
    if(!token()&&attempt<12){setTimeout(()=>handleReturn(attempt+1),500);return;}
    if(!token()){notice('Inicia sesión para confirmar el estado del pago.','error');return;}
    try{const r=await fetch(`${API()}/api/payments/stripe/confirm-session`,{method:'POST',headers:authHeaders(),body:JSON.stringify({sessionId})});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'No se pudo confirmar el pago');notice(d.paid?'Pago de prueba confirmado.':'El pago sigue procesándose.');cleanPaymentQuery();await load();}catch(e){notice(e.message,'error');}
  }

  function cleanPaymentQuery(){
    const u=new URL(location.href);u.searchParams.delete('payment');u.searchParams.delete('session_id');history.replaceState({},'',u.pathname+u.search+u.hash);
  }
